...
extern SemaphoreHandle_t OLEDSemaphore;
...
void OLED_Reset(void)
{
	...
	if (is_resetting == 1)
		return; // 如果正在复位中又发生错误，直接放弃，防止套娃
	is_resetting = 1;

	// 1. 先关闭单片机内部的 I2C 外设
	HAL_I2C_DeInit(&hi2c1);

	// 2. 【防御2】：将 PB6(SCL) 和 PB7(SDA) 临时配置为普通开漏输出 GPIO
	...
	// 3. 强行制造 9 个时钟脉冲，逼迫 OLED 内部状态机复位并松开 SDA 线！
...
void OLED_WriteCommand(uint8_t Command)
{
	...
	while (HAL_I2C_GetState(&hi2c1) != HAL_I2C_STATE_READY)
	{
		...
		if (I2C_NUM == 0)
		{
			OLED_Reset(); // 如果等待超时，重置I2C外设
			return;
		}
	}
	if (HAL_I2C_Mem_Write(&hi2c1, 0x78, 0x00, I2C_MEMADD_SIZE_8BIT, &Command, 1, 100) != HAL_OK)
	{
		OLED_Reset(); // 如果发送命令失败，重置I2C外设
	}
}
...
void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
	...
	if (HAL_I2C_Mem_Write(&hi2c1, 0x78, 0x40, I2C_MEMADD_SIZE_8BIT, Data, Count, 100) != HAL_OK)
	{
		OLED_Reset(); // 如果发送命令失败，重置I2C外设
	}
...
void OLED_Init(void)
{
	...
	OLED_WriteCommand(0x00); // 0x00: 水平寻址模式 (这是 DMA 能够一波流发完1024字节的关键!)
