...
void GUI_Task(void *pvParameters)
{
    ...
    // 2. 后台开始连网并对时
    WI_FI_Init();      // 连接路由器并复位、配置 SNTP

    // 循环死缠烂打法
    uint8_t retry = 0;
    while(retry < 15) // 最多循环询问 15 次（15秒）
    {
        vTaskDelay(pdMS_TO_TICKS(1000)); // 每次等 1 秒
        ESP8266_GetTime(); // 发指令获取，内部会自动调用 Parse_Time

        // 检查缓存：如果没有 1970 并且包含了时间标志，说明拿到真时间了！
        if (strstr((char *)UART_Rx_Buffer, "1970") == NULL && strstr((char *)UART_Rx_Buffer, "+CIPSNTPTIME:") != NULL)
        {
            break; // 成功拿到真时间，立刻跳出循环！
        }
        retry++;
    }
    ...
    while (1)
    {
        ...
        else
        {
            ...
            if (xQueueReceive(Queue_Key, &Current_Key_Msg, wait_time) != pdTRUE)
            {
                ...
                // 【每小时静默对时逻辑】
                if (stopwatch_flag == 0 && CurrMode != 13) 
                {
                    hour_sync_tick++;
