...
{
    HAL_UARTEx_ReceiveToIdle(&huart2, Buff, 1024, UART_Rx_Len, 1000);
}
...
void ESP8266_SendAt(const uint8_t *At, uint16_t At_Len, uint8_t *Buff, uint16_t *UART_Rx_Len)
{
    ...
    HAL_UART_Transmit(&huart2, At, At_Len, 100);

    if (strstr((char *)At, "+++") != NULL || strstr((char *)At, "GET") != NULL)
    {
        HAL_UARTEx_ReceiveToIdle(&huart2, Buff, 1024, UART_Rx_Len, 2000);
    }
    else
    {
        uint16_t TIMEOUT = 0;
...
void Parse_Time(void)
{
    ...
    if (p != NULL)
    {
        ...
        // 定义变量存放解析出来的数据
        char week_str[4], month_str[4];
        int day, hour, minute, second, year;

        // 使用 sscanf 提取所有的年月日时分秒！
        // 匹配格式例如：Fri Apr 17 18:30:00 2026
        if (sscanf(p + 13, "%3s %3s %d %d:%d:%d %d", week_str, month_str, &day, &hour, &minute, &second, &year) == 7)
        {
            // 1. 将英文月份转换成数字 (Jan -> 1)
            uint8_t month_num = 1;
            ...
            sDate.Year = (uint8_t)(year % 100); // 只要年份的后两位，比如 2026 年变成 26
            sDate.Month = month_num;
            sDate.Date = (uint8_t)day;
            sDate.WeekDay = week_num;

            // 5. 连续写入 STM32 的 RTC 硬件
            HAL_RTC_SetTime(&hrtc, &sTime, RTC_FORMAT_BIN); // 先写时间
            HAL_RTC_SetDate(&hrtc, &sDate, RTC_FORMAT_BIN); // 再写日期
        }
    }
}
